...
__global__ void upsample_nearest2d_nhwc_out_frame(
  ...
  if (index < out_numel) {
    ...
    odata[index] = idata[at::native::idx_cl(n, h1, w1, c, height1, width1, channels)];
  }
}
  ...
  if (index < gi_numel) {
    ...
    for (int ih = h1; ih < h1_up; ih++) {
      for (int iw = w1; iw < w1_up; iw++) {
        grad += go[at::native::idx_cl(n, ih, iw, c, height1, width1, channels)];
      }
    }
...
static void upsample_nearest2d_out_cuda_template(
  ...
  at::TensorArg input_arg{input_, "input_", 1}, output_arg{output, "output", 2};
  at::checkAllSameGPU(__func__, {input_arg, output_arg});
  ...
  const float height_scale = at::native::compute_scales_value<float>(scales_h, input_height, output_height);
  const float width_scale = at::native::compute_scales_value<float>(scales_w, input_width, output_width);
  ...
  if (memory_format == at::MemoryFormat::ChannelsLast && channels >= 4 && \
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "upsample_nearest2d_nhwc_out_frame", [&] {
      ...
      upsample_nearest2d_nhwc_out_frame<scalar_t, nn_compute_source_index_fn>
        <<<at::ceil_div(num_kernels, num_threads), num_threads, 0, at::cuda::getCurrentCUDAStream()>>>(
          idata,
          odata,
          channels,
          input_height,
    ...
    at::Tensor output_c = output.is_contiguous() ? output : at::empty(output.sizes(), output.options());
    ...
    int block_x = std::min<int>(
        maxThreadsDim[0], std::min<int>(at::native::lastPow2(output_width), max_threads));
    int block_y = std::min<int>(
        maxThreadsDim[1],
        std::min<int>(at::native::lastPow2(output_height), max_threads / block_x));
    int block_z = std::min<int>(
        maxThreadsDim[2], std::min<int>(nc, max_threads / block_x / block_y));
    const dim3 block(block_x, block_y, block_z);

    int grid_x = at::ceil_div(output_width, block_x);
    int grid_y = at::ceil_div(output_height, block_y);
    int grid_z = std::min<int>(
        maxGridSize[2], at::ceil_div(nc, block_z * 4));
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "upsample_nearest2d_out_frame", [&] {
...
static void upsample_nearest2d_backward_out_cuda_template(
  ...
  at::checkAllSameGPU(__func__, {grad_output_arg, grad_input_arg});
  ...
  const float height_scale = at::native::compute_scales_value_backwards<float>(scales_h, output_height, input_height);
  const float width_scale = at::native::compute_scales_value_backwards<float>(scales_w, output_width, input_width);
  ...
  if (memory_format == at::MemoryFormat::ChannelsLast && channels >= 4 && \
    ...
    at::Tensor grad_output = grad_output_.contiguous(at::MemoryFormat::ChannelsLast);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, grad_output.scalar_type(), "upsample_nearest2d_backward_nhwc_out_frame", [&] {
      using accscalar_t = at::acc_type<scalar_t, true>;
    ...
    dim3 bdim{std::min<unsigned int>(
        at::cuda::getCurrentDeviceProperties()->maxThreadsPerBlock, MAX_THREADS)};
    dim3 gdim{at::ceil_div(n, bdim.x)};
    // safe check for int32 indexing; implicitly restrict launch config for kernel
    TORCH_CHECK(grad_input.numel() <= std::numeric_limits<int32_t>::max());

    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, grad_output.scalar_type(), "upsample_nearest2d_backward_out_frame", [&] {
      using accscalar_t = at::acc_type<scalar_t, true>;
...
void upsample_nearest2d_out_cuda (
    const at::Tensor& input,
    c10::IntArrayRef output_size,
    c10::optional<double> scales_h,
    c10::optional<double> scales_w,
    const at::Tensor& output) {
  upsample_nearest2d_out_cuda_template<at::native::nearest_neighbor_compute_source_index>(
      output, input, output_size, scales_h, scales_w);
}

void upsample_nearest2d_backward_out_cuda(
    ...
    c10::optional<double> scales_w,
    const at::Tensor& grad_input) {
  upsample_nearest2d_backward_out_cuda_template<at::native::nearest_neighbor_bw_compute_source_index>(
